...
/**
 * La page publique d'un équipement partagé.
 *
 * Le second segment n'est **pas** lu : c'est le nom de l'équipement au moment du
 ...
 * (voir `buildShare`).
 */
export default async function SharedLoadoutPage({
                                                  params,
                                              }: {
    params: Promise<{id: string}>;
}) {
    const {id} = await params;
    const share = await readShare("loadout", id);
    if (!share) notFound();
